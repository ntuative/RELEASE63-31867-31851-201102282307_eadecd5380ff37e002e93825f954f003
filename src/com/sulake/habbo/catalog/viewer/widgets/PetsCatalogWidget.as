package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : Boolean
      {
         ...
         this.var_463 = this.getPetTypeIndexFromProduct(_loc3_.localizationId);
         if(this.var_463 >= 8)
         {
            return false;
         }
         ...
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc5_ = _loc4_.avatarRenderManager.petDataManager.getPetData(this.var_463,this);
               if(_loc5_)
               {
                  this.var_332 = _loc5_.colors;
         ...
         this._offers.add(this.var_463,_loc3_);
         events.addEventListener(WidgetEvent.CWE_SELECT_PRODUCT,this.onSelectProduct);
         events.addEventListener(WidgetEvent.CWE_COLOUR_INDEX,this.onColourIndex);
         events.addEventListener(WidgetEvent.const_811,this.onApproveNameResult);
      ...
      public function onDropMenuEvent(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(param1.type == WindowEvent.const_58)
         {
            ...
            if(this.var_188 == null || _loc3_ >= this.var_188.length)
            {
               return;
            }
            this.var_681 = _loc3_;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         if(this.var_476 < 0 || this.var_476 > this.var_332.length)
         {
            this.var_476 = 0;
         }
         this.updateImage();
      ...
      private function onSellablePetBreeds(param1:CatalogWidgetSellablePetBreedsEvent) : void
      {
         ...
         {
            this.var_188 = this.parseSellableBreeds(param1.sellableBreeds);
            this.var_681 = 0;
      ...
      private function parseSellableBreeds(param1:Array) : Array
      {
         ...
         {
            if(_loc3_.type == this.var_463 && _loc3_.sellable)
            {
      ...
      private function updateBreedSelections() : void
      {
         ...
         {
            _loc5_ = this.var_188[_loc3_] as int;
            _loc2_.push(_loc1_.getKey(this.getRaceLocalizationKey(this.var_463,_loc5_),this.getRaceLocalizationKey(this.var_463,_loc5_)));
            _loc3_++;
         }
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_188 == null || this.var_681 >= this.var_188.length)
         {
            return;
         }
         var _loc2_:* = 0;
         if(this.var_476 >= 0 && this.var_476 < this.var_332.length)
         {
            _loc7_ = this.var_332[this.var_476] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:int = this.var_188[this.var_681] as int;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_463,_loc3_,_loc2_,AvatarScaleType.const_55,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_305,AvatarAction.const_839);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_476 >= this.var_332.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_332[this.var_476] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:int = this.var_188[this.var_681] as int;
      ...
      private function updateAvailableBreeds(param1:String) : void
      {
         ...
         if(this.var_188 == null)
         {
            ...
            this.var_188 = this.parseSellableBreeds(_loc2_);
            this.var_681 = 0;
            if(this.var_188 != null)
            {
               this.updateBreedSelections();
