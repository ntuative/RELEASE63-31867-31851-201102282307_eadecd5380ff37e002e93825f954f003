package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategory implements IDisposable
   {
      ...
      {
         var _loc4_:* = null;
         if(this.var_202.indexOf(param1) >= 0)
         {
            return;
         }
         ...
         {
            _loc4_ = this.var_202[_loc3_];
            if(_loc2_ < _loc4_.name.toLowerCase())
            {
               this.var_202.splice(_loc3_,0,param1);
               return;
            }
            _loc3_++;
         }
      ...
      public function removeFriend(param1:int) : Friend
      {
         ...
         if(_loc2_ != null)
         {
            Util.remove(this.var_202,_loc2_);
            return _loc2_;
         }
         return null;
      }
      ...
      {
         if(this.var_814 >= this.getPageCount())
         {
            this.var_814 = Math.max(0,this.getPageCount() - 1);
         }
      }
      
      public function getPageCount() : int
      {
         return Math.ceil(this.var_202.length / const_376);
      }
      
      public function getStartFriendIndex() : int
      {
         this.checkPageIndex();
         return this.var_814 * const_376;
      }
      
      public function getEndFriendIndex() : int
      {
         this.checkPageIndex();
         return Math.min((this.var_814 + 1) * const_376,this.var_202.length);
      }
