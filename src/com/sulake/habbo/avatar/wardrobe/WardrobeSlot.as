package com.sulake.habbo.avatar.wardrobe
{
   ...
   public class WardrobeSlot implements IOutfit, IAvatarImageListener
   {
      ...
      private var var_373:IBitmapWrapperWindow;
      ...
      private function createView() : void
      {
         ...
         this.var_373 = this._view.findChildByName("image") as IBitmapWrapperWindow;
      }
      ...
      public function updateView() : void
      {
         ...
         if(this.var_645 && this.var_1481)
         {
            _loc5_ = this._controller.avatarRenderManager.createAvatarImage(this.figure,AvatarScaleType.const_90,this.var_987,this);
            if(_loc5_)
            {
               _loc5_.setDirection(AvatarSetType.const_34,parseInt(FigureData.const_809));
         ...
         if(this.var_373)
         {
            if(this.var_373.bitmap)
            {
               this.var_373.bitmap.dispose();
            }
            this.var_373.bitmap = new BitmapData(this.var_373.width,this.var_373.height,true,0);
            _loc7_ = (this.var_373.width - _loc1_.width) / 2;
            _loc8_ = (this.var_373.height - _loc1_.height) / 2;
            this.var_373.bitmap.draw(_loc1_,new Matrix(1,0,0,1,_loc7_,_loc8_));
         }
         if(_loc2_)
         {
            _loc1_.dispose();
         ...
         if(_loc4_)
         {
            _loc4_.visible = this.var_1481 && this.var_645 != null;
         }
      }
      ...
      private function eventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         switch(param2.name)
         {
            case "set_button":
               ...
               this.var_987 = this._controller.gender;
               this._controller.handler.saveWardrobeOutfit(this.var_1936,this);
               this.updateView();
               break;
            case "get_button":
               if(this.var_645)
               {
                  this._controller.loadAvatarInEditor(this.var_645,this.var_987,this._controller.clubMemberLevel);
               }
         }
      }
