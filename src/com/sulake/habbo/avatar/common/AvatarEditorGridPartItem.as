package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private const const_1957:int = 13421772;
      
      private const const_1958:int = 16777215;
      
      private const const_333:Array = [2,6,0,4,3,1];
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 != null)
         {
            ...
            {
               this.var_1893 = Math.max(this.var_1893,_loc6_.colorLayerIndex);
            }
         }
         this.var_432 = this.var_66.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_651 != null && !this.var_1894)
            {
               _loc3_ = this.var_651;
            }
            else
            ...
            if(this.var_772)
            {
               switch(this.var_772.clubLevel)
               {
                  ...
                  case HabboClubLevelEnum.const_40:
                     _loc2_.style = HabboIconType.const_638;
                     break;
                  case HabboClubLevelEnum.const_41:
                     _loc2_.style = HabboIconType.const_780;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this._window.color = this.const_1957;
         }
         else
         {
            this._window.color = this.const_1958;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_432.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_432.isFigureReady(_loc1_))
         {
            this.var_432.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_333[_loc6_] + "_" + FigureData.const_155;
               _loc4_ = this.var_432.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
               while(!_loc7_ && _loc6_ < this.const_333.length)
               {
                  _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_333[_loc6_] + "_" + FigureData.const_155;
                  _loc4_ = this.var_432.getAssetByName(_loc3_) as BitmapDataAsset;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_634 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1892)
               {
                  ...
                  this.var_1892 = _loc9_.content as BitmapData;
               }
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               if(this.var_1894 && _loc2_.colorLayerIndex > 0)
               {
