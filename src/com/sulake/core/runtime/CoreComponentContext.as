package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1303:String = "asset-libraries";
      
      private static const const_1304:String = "library";
      
      private static const const_1301:String = "service-libraries";
      
      private static const const_1308:String = "library";
      
      private static const const_1302:String = "component-libraries";
      
      private static const const_1306:String = "library";
      
      private static const const_1305:String = "error_data";
      
      private static const const_1307:String = "error_cat";
      
      private static const const_1300:String = "error_desc";
       
      
      private var var_348:LibraryLoaderQueue;
      
      private var var_708:IEventDispatcher;
      
      private var var_490:uint;
      
      private var var_407:Function;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         switch(param2 & 0)
         {
            case Core.const_1074:
               debug("Core; using simple frame update handler");
               this.var_407 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_805:
               debug("Core; using complex frame update handler");
               this.var_407 = this.complexFrameUpdateHandler;
               break;
            case Core.const_1457:
               debug("Core; using profiler frame update handler");
               this.var_407 = this.profilerFrameUpdateHandler;
               this.var_237 = new Profiler(this);
               attachComponent(this.var_237,[new IIDProfiler()]);
               var_443.addChild(new ProfilerViewer(this.var_237));
               break;
            case Core.const_897:
               debug("Core; using experimental frame update handler");
               this.var_407 = this.experimentalFrameUpdateHandler;
               break;
            case Core.CORE_SETUP_DEBUG:
               debug("Core; using debug frame update handler");
               this.var_407 = this.debugFrameUpdateHandler;
         }
      }
      
      public function getNumberOfFilesPending() : uint
      {
         return this.var_348.length;
      }
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1305,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1307,String(param3));
         ErrorReportStorage.setParameter(const_1300,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_708 = loadingEventDelegate;
         if(this.var_348 == null)
         {
            this.var_348 = new LibraryLoaderQueue(_debug);
         }
         try
         {
            node = config.child(const_1303)[0];
            if(node != null)
            {
               list = node.child(const_1304);
         ...
         {
            error("Failed to parse asset libraries from config xml!",true,Core.const_275,e);
         }
         try
         {
            node = config.child(const_1301)[0];
            if(node != null)
            {
               list = node.child(const_1308);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse interfaces from config xml!",true,Core.const_275,e);
         }
         try
         {
            node = config.child(const_1302)[0];
            if(node != null)
            {
               list = node.child(const_1306);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse components from config xml!",true,Core.const_275,e);
         }
         if(!disposed)
         {
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         this.error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1413);
         if(!disposed)
         {
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_708)
         {
            this.var_708.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_708)
         {
            ...
            this.var_708.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_708)
                  {
                     this.var_708.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_490 - this.var_348.length,this.var_490,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         if((this.var_1358 & 0) == Core.const_897)
         {
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         this.var_407(_loc2_,_loc2_ - this._lastUpdateTimeMs);
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         while(priority < CoreComponentContext.const_124)
         {
            ...
            {
               if(this.var_589[priority] < priority)
               {
                  ++this.var_589[priority];
                  skip = true;
               }
            }
            if(!skip)
            {
               this.var_589[priority] = 0;
               receivers = this.var_408[priority];
               index = 0;
               length = receivers.length;
               while(index != length)
               {
